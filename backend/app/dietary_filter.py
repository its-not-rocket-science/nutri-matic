...
def food_dietary_status(food: Food, db: Session, user: User | None) -> Suitability | None:
    """For display, not filtering — None (render no badge) for anonymous
    callers or a user with no dietary_pattern/constraints set, same
    ...
    if not user.dietary_pattern and not constraint_tags:
        return None
    return evaluate_food(food.name, food.data_type, user.dietary_pattern, constraint_tags)


def foods_dietary_status(foods: list[Food], db: Session, user: User | None) -> dict[int, Suitability]:
    ...
    for food in foods:
        suitability = evaluate_food(food.name, food.data_type, user.dietary_pattern, constraint_tags)
...
def recipes_dietary_status(recipes: list[Recipe], db: Session, user: User | None) -> dict[int, Suitability]:

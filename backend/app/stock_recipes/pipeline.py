...
def _match_candidate(db: Session, cand: dict, min_coverage: float) -> None:
    ...
    for parsed in parsed_ingredients:
        ...
        row: dict[str, Any] = {
            ...
            "preferred_fdc_id": match.preferred_fdc_id,
            "preferred_food_id": match.preferred_food_id,
            "used_fallback": match.used_fallback,
            "validation_warning": match.validation_warning,
...
def _import_one(
    ...
    for m in row["matches"]:
        ...
        db.add(models.RecipeIngredientProvenance(
            ...
            match_rationale=m.get("rationale"),
            match_preferred_fdc_id=m.get("preferred_fdc_id"), match_preferred_food_id=m.get("preferred_food_id"),
            match_used_fallback=m.get("used_fallback"), match_validation_warning=m.get("validation_warning"),

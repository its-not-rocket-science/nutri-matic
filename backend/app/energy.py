...
def calculate_age(profile: Profile, *, current_year: int | None = None) -> int | None:
...
def calculate_eer(profile: Profile, *, current_year: int | None = None) -> float | None:
    ...
    if None in (profile.weight_kg, profile.height_cm, profile.birth_year, profile.sex, profile.activity_level):
        return None

    age = calculate_age(profile, current_year=current_year)

    bmr = 10 * profile.weight_kg + 6.25 * profile.height_cm - 5 * age
    bmr += 5 if profile.sex == "male" else -161

    eer = bmr * ACTIVITY_MULTIPLIERS[profile.activity_level]

...
| **Audit logging** | **Partial, and not what this row originally claimed.** Operational-hardening work since this row was written added structured `logging` throughout the backend (16 modules — demo lifecycle, shared rate limiting, monitoring, recommendation/error signals; see `docs/monitoring.md`). Caught by PR review: this only reliably reaches a sink for **`WARNING`/`ERROR` events** — `app/monitoring.py`'s Sentry integration only processes `WARNING`+, and the running server (unlike `demo_purge.py`'s/`recommend_validate.py`'s own standalone CLI entrypoints, which call `logging.basicConfig()` themselves) attaches no handler of its own, so an `INFO`-level call like `recommendation_request`'s own success-path log line reaches neither stdout nor Sentry today — `logging.lastResort`'s fallback is `WARNING`+ only. None of this, at any level, is a persisted, queryable **who-accessed-which-record-when** audit trail. | The operational-logging gap this row originally described is closed for warning/error-level operational signals and for CLI commands that configure their own handler; ordinary `INFO`-level request logging is still effectively going nowhere, and the actual audit-trail gap for this audience is not addressed at all. Hospitals/care homes still need: structured access logging (user/API-key id, record id, endpoint, timestamp, outcome) persisted somewhere queryable — not stdout/Sentry, which age out, aren't guaranteed to capture `INFO`-level events as noted above, and aren't structured for "show me every access to resident X's data" — plus specific audit entries for clinician-dashboard access (Phase 4.2's `ClinicianClientLink`) given that's the existing feature closest to "someone viewing someone else's health data." |

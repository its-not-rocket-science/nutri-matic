...
| **Audit logging** | **Partial, and not what this row originally claimed.** Operational-hardening work since this row was written added structured `logging` throughout the backend (16 modules — demo lifecycle, shared rate limiting, monitoring, recommendation/error signals; see `docs/monitoring.md`) and wired it to Sentry when configured. But every one of those is an *operational* event (errors, rate-limit hits, request latency) written to stdout/an APM tool — none is a persisted, queryable **who-accessed-which-record-when** audit trail. | The operational-logging gap this row originally described is closed; the actual audit-trail gap for this audience is not. Hospitals/care homes still need: structured access logging (user/API-key id, record id, endpoint, timestamp, outcome) persisted somewhere queryable — not stdout/Sentry, which age out and aren't structured for "show me every access to resident X's data" — plus specific audit entries for clinician-dashboard access (Phase 4.2's `ClinicianClientLink`) given that's the existing feature closest to "someone viewing someone else's health data." |

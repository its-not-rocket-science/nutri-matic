...
| 22 | Historical git-history exposure of the 14 files quarantined by PROMPT 9 | Code-enforced (branch history) + manual residual (GitHub-retained PR history) | `docs/phytate-review/PRIVATE_ARTIFACTS.md`'s "Historical git-history exposure (PROMPT 15, resolved 2026-08-25)"; `prompts.txt` OPTIONAL PROMPT 15 | ✅ Resolved for `refs/heads/*` (2026-08-25) — `git filter-repo` purged all 14 files from every commit on `main` and its affected branches, verified by path scan and a full content-fingerprint scan of all 3,580 reachable blobs (one hit, confirmed by SHA-256 to be the reviewed synthetic fixture, not real data); tree-diffed identical to pre-rewrite `main`. ⚠️ NOT resolved for PRs #41, #43, #52, #54, #55, #57 — GitHub retains their pre-squash history server-side (`refs/pull/N/head`), unaffected by a force-push to `main` | Paul: file a GitHub Support request to purge cached views/backups for PR numbers #41, #43, #52, #54, #55, #57 specifically — that is the one remaining action, and only GitHub itself can do it |
